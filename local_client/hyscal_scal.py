...
for i in range(200, 300):
    k = requests_data[i]
    os.system("jmeter -n -t newplan.jmx -l hyscal/jmeter_rawdata_%s.csv -JthreadNum=%s" % (i, k))
    # run requests_server.py
    if i > 204:
        with open('hyscal_adapter.py', 'r') as f:
            exec(f.read())
            f.close()
    jmeter = pd.read_csv('hyscal/jmeter_rawdata_%s.csv' % i)

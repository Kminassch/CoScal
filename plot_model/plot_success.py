...
for i in range(200, 300):
    ...
    data = data['success'].astype('int')
    data_ad = data_ad['success'].astype('int')
    data_k8s = data_k8s['success'].astype('int')
    data_hyscal = data_hyscal['success'].astype('int')
...
plt.legend()
plt.savefig('plotfig/success_compare.png')
...
j_rawdata.to_csv('dataset/success.csv')
j_rawdata_ad.to_csv('dataset/success_ad.csv')
j_rawdata_k8s.to_csv('dataset/success_k8s.csv')
j_rawdata_hyscal.to_csv('dataset/success_hyscal.csv')
